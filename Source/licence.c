...
licence_generate_keys(rdcConnection conn, uint8 * client_random, uint8 * server_random, uint8 * pre_master_secret)
{
	...
	memcpy(conn->licenseSignKey, key_block, 16);

	/* Generate RC4 key from next 16 bytes */
	sec_hash_16(conn->licenseKey, &key_block[16], client_random, server_random);
}
...
licence_process_demand(rdcConnection conn, STREAM s)
{
	...
	licence_generate_keys(conn, null_data, server_random, null_data);

	licence_size = load_licence(&licence_data);
	if (licence_size > 0)
	{
		/* Generate a signature for the HWID buffer */
		licence_generate_hwid(conn, hwid);
		sec_sign(signature, 16, conn->licenseSignKey, 16, hwid, sizeof(hwid));

		/* Now encrypt the HWID */
		RC4_set_key(&crypt_key, 16, conn->licenseKey);
		RC4(&crypt_key, sizeof(hwid), hwid, hwid);

		licence_present(conn, null_data, null_data, licence_data, licence_size, hwid, signature);
		xfree(licence_data);
		return;
	}

	licence_send_request(conn, null_data, null_data, conn->username, conn->hostname);
}
...
licence_process_authreq(rdcConnection conn, STREAM s)
{
	...
	sec_sign(out_sig, 16, conn->licenseSignKey, 16, sealed_buffer, sizeof(sealed_buffer));

	/* Now encrypt the HWID */
	RC4_set_key(&crypt_key, 16, conn->licenseKey);
	RC4(&crypt_key, LICENCE_HWID_SIZE, hwid, crypt_hwid);

	licence_send_authresp(conn, out_token, crypt_hwid, out_sig);
}
