...
void
rdpsnd_dsp_softvol(unsigned char *buffer, unsigned int size, RDWaveFormat * format)
{
...
void
rdpsnd_dsp_swapbytes(unsigned char *buffer, unsigned int size, RDWaveFormat * format)
{
	int i;
	uint8 swap;
...
RD_BOOL
rdpsnd_dsp_resample_supported(RDWaveFormat * format)
{
...
rdpsnd_dsp_resample(unsigned char **out, unsigned char *in, unsigned int size,
		    RDWaveFormat * format, RD_BOOL stream_be)
{
#ifdef HAVE_LIBSAMPLERATE
		   ...
		   RDWaveFormat * format)
{
    ...
    /* Commented out until move sound code is moved from rdesktop 1.6
