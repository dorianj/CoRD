...
serial_enum_devices(rdcConnection conn, uint32 * id, char *optarg)
{
	...
	while ((pos = next_arg(optarg, ',')) && *id < RDPDR_MAX_DEVICES)
	{
		...
		printf("SERIAL %s to %s\n", conn->rdpdrDevice[*id].name,
		       conn->rdpdrDevice[*id].local_path);
		/* set device type */
		conn->rdpdrDevice[*id].device_type = DEVICE_TYPE_SERIAL;
		conn->rdpdrDevice[*id].pdevice_data = (void *) pser_inf;
		count++;
		(*id)++;
	      ...
	      uint32 flags_and_attributes, char *filename, NTHANDLE * handle)
{
	...
	{
		printf("INFO: SERIAL %s access denied\n", conn->rdpdrDevice[device_id].name);
		fflush(stdout);
		return STATUS_ACCESS_DENIED;
	}

	/* Store handle for later use */
	conn->rdpdrDevice[device_id].handle = serial_fd;

	/* some sane information */
	DEBUG_SERIAL(("INFO: SERIAL %s to %s\nINFO: speed %u baud, stop bits %u, parity %u, word length %u bits, dtr %u, rts %u\n", conn->rdpdrDevice[device_id].name, conn->rdpdrDevice[device_id].local_path, pser_inf->baud_rate, pser_inf->stop_bits, pser_inf->parity, pser_inf->word_length, pser_inf->dtr, pser_inf->rts));
...
serial_close(rdcConnection conn, NTHANDLE handle)
{
	...
	rdpdr_abort_io(conn, handle, 0, STATUS_TIMEOUT);
	close(handle);
	return STATUS_SUCCESS;
}
...
serial_device_control(rdcConnection conn, NTHANDLE handle, uint32 request, STREAM in, STREAM out)
{
	...
	switch (request)
	{
		...
		case SERIAL_IMMEDIATE_CHAR:
			...
			serial_write(conn, handle, &immediate, 1, 0, &result);
			break;
		...
		case SERIAL_PURGE:
			...
			if (purge_mask & SERIAL_PURGE_TXABORT)
				rdpdr_abort_io(conn, handle, 4, STATUS_CANCELLED);
			if (purge_mask & SERIAL_PURGE_RXABORT)
				rdpdr_abort_io(conn, handle, 3, STATUS_CANCELLED);
			break;
		case SERIAL_WAIT_ON_MASK:
			DEBUG_SERIAL(("serial_ioctl -> SERIAL_WAIT_ON_MASK %X\n",
				      pser_inf->wait_mask));
			pser_inf->event_pending = 1;
			if (serial_get_event(conn, handle, &result))
			{
...
serial_get_timeout(rdcConnection conn, NTHANDLE handle, uint32 length, uint32 * timeout, uint32 * itv_timeout)
{
	...
	if (conn->rdpdrDevice[index].device_type != DEVICE_TYPE_SERIAL)
	{
		return False;
	}
